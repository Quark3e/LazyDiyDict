#pragma once
#ifndef HPP_LAZYDIYDICT
#define HPP_LAZYDIYDICT


#ifndef CONFIG__HEADER_ONLY
#define CONFIG__HEADER_ONLY false
#endif //CONFIG__HEADER_ONLY
...
namespace LazyDiyDict {
    std::string _info_name__namespace = "LazyDiyDict";
     ...
     */
    template<typename T> bool hasRepetitions(std::list<T> lst) {
        bool repeated = false;
        for(T elem: lst) {
            int foundCount = 0;
            for(T var: lst) {
    ...
    template<typename _check> int check_existence(_check key, std::vector<_check> vec) {
        for(size_t i=0; i<vec.size(); i++) {
            if(key==vec.at(i)) return static_cast<int>(i);
        }
        return -1;
    }
    template<typename _check> int check_existence(_check key, std::list<_check> lst) {
        int idx=0;
    ...
    template<typename _check> int check_existence(_check key, const _check arr[], int arrLen) {
    ...
    class dict {
        ...
        public:
             ...
             */
            int operator[] (std::string key) { return check_existence<std::string>(key, this->_keys); }
    ...
    template<typename _key_type, typename _store_type>
    struct _pair {
        _key_type   key;
        _store_type value;
